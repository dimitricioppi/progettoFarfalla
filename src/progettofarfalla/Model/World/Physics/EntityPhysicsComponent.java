...
public class EntityPhysicsComponent extends PhysicsComponent {
    ...
    public void update(long dt, GameObject obj, World w) {
        ...
        Entity killer = (Entity) obj;

        Optional<GameObject> hit = w.checkCollisionWithEnemy(killer.getCurrentPos(), bbox);

        if (hit.isPresent() && killer.getType() == Entity.Type.BAT_KILLER){

            w.notifyWorldEvent(new KillEnemyEvent(obj,hit.get()));
        }          
    }
}