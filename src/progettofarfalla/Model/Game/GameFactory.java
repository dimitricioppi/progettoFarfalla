 ...
 * @author Daniela
 */
import progettofarfalla.Model.Imputs.PlayerInputComponentWithAcc;
import progettofarfalla.Model.Imputs.NullInputComponent;
...
import progettofarfalla.Model.Game.GameObjects.PickUp.Type;
...
public class GameFactory {
    ...
    public Entity createButterfly(P2d pos, double radius, V2d vel, Player player){
        ...
        return new Entity(Entity.Type.BUTTERFLY, pos, vel, new CircleBoundingBox(new P2d(pos.getX(), pos.getY()), radius),
                        ...
                        new ButterflyPhysicsComponent(), radius);
    }

    public Entity createEnemy(Entity.Type t,P2d pos, double radius, V2d vel, InputComponent input, GraphicsComponent graph){
    ...
    public PickUp createPickUpObject(Type t,P2d pos, double edge, GraphicsComponent graph){

        return new PickUp(t, pos, new V2d(0,0), new CircleBoundingBox(new P2d(pos.getX(), pos.getY()), edge/2),
                    new NullInputComponent(),
                    graph,
                    new PickUpObjPhysicsComponent(), edge/2);
    }

    public Summoning createSummoning(P2d pos, double radius, Entity.Type type, World world){
        
        return new Summoning(Summoning.Type.SUMMON, pos, new V2d(0,0), new CircleBoundingBox(new P2d(pos.getX(), pos.getY()), radius),
                        new NullInputComponent(),
                        new SummoningGraphicsComponent(),
                        new SummoningPhysicsComponent(), radius,
                        type, world);

    }

    public Entity createKiller(P2d pos, double radius, V2d vel, InputComponent input){
    ...
    public Button createButton(P2d pos, double height, double base, Button.Type type, GameEngine engine){

        return new Button(type, pos, new NullInputComponent(),
                        new ButtonGraphicsComponent() ,
                        new ButtonPhysicsComponent(),
                        base, height, engine);
    }
}
