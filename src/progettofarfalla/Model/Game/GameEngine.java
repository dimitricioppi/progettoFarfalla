...
import progettofarfalla.Model.Imputs.*;
import java.util.*;
...
public class GameEngine implements WorldEventListener {
    ...
    public void initGame(){
        ...
        this.view = new SwingScene(gameState, this, 750, 700, 45, 45);
    }
    ...
    private void processInput(){
        ...
        for (GameObject bat: gameState.getWorld().getBats()) {
    ...
    private void checkEvents(){
        ...
        eventQueue.stream().forEach(ev -> {
            ...
            } else if (ev instanceof HitBatEvent btev){

                InputComponent input = btev.getSelf().getInputComponent();
                Entity collidingBat = (Entity) btev.getCollisionObj();
                if (input instanceof AbstractPlayerInputComponent && collidingBat.getType() != Entity.Type.BAT_KILLER) {

                    ((AbstractPlayerInputComponent) input).getPlayer().updateLives(-1);                 				
                }

                if (scene.getBats().contains(collidingBat)){

                    scene.removeEnemy(collidingBat);
                    collidingBat.getCurrentVel().killVel();
                }

            } else if (ev instanceof KillEnemyEvent kbev){

                scene.removeEnemy(kbev.getCollisionObj());
                kbev.getCollisionObj().getCurrentVel().killVel();
                scene.removeKiller(kbev.getSelf());
    ...
    public void eventReaction(PickUp p, InputComponent input){
        ...
        if(null != p.getType())switch (p.getType()) {
            ...
            case PICKABLE_OBJ -> {
                ((AbstractPlayerInputComponent) input).getPlayer().updateScore(10*(this.gameState.getWorld().getBats().size()+1));
                ((AbstractPlayerInputComponent) input).getPlayer().updatePickUpsTaken(1);
                this.gameState.getWorld().addPickUp(f.createPickUpObject(PICKABLE_OBJ, new P2d(this.gameState.getWorld()), 1, new PickUpObjGraphicsComponent(false)));
            }

            case EXTRA_POINTS -> {
                ((AbstractPlayerInputComponent) input).getPlayer().updateScore(30*(this.gameState.getWorld().getBats().size()+1));
                ((AbstractPlayerInputComponent) input).getPlayer().updatePickUpsTaken(1);
            }

            case CURSE ->             {
                ((AbstractPlayerInputComponent) input).getPlayer().updateScore(100*(this.gameState.getWorld().getBats().size()+1));
                ((AbstractPlayerInputComponent) input).getPlayer().updatePickUpsTaken(1);
                {
                    gameState.getWorld().addSummoning(f.createSummoning(p.getCurrentPos(), 1, DRAGONFLY, gameState.getWorld()));
                }
            }

            case SUPER -> {
                ((AbstractPlayerInputComponent) input).getPlayer().updateScore(150*(this.gameState.getWorld().getBats().size()+1));
                ((AbstractPlayerInputComponent) input).getPlayer().updatePickUpsTaken(1);
            }
            
            case LIFE -> {
                ((AbstractPlayerInputComponent) input).getPlayer().updateScore(50*(this.gameState.getWorld().getBats().size()+1));
                ((AbstractPlayerInputComponent) input).getPlayer().updateLives(1);
                ((AbstractPlayerInputComponent) input).getPlayer().updatePickUpsTaken(1);
            }
            ...
            case BOMB -> {
                ...
                ((AbstractPlayerInputComponent) input).getPlayer().updatePickUpsTaken(1);
                if(!gameState.getWorld().getBats().isEmpty()){

                    gameState.getWorld().addKiller(f.createKiller( p.getCurrentPos(), 0.5, new V2d(0,0), new KillerInputComponent(gameState.getWorld().getClosestBat())));
    ...
    public void createGameObjects(int picksTaken){
        ...
        if(picksTaken%this.rand.nextInt(20, 25) == 0 && picksTaken != 0){           
        ...
        if(picksTaken%this.rand.nextInt(2, 5) == 0 && picksTaken != 0){           
            ...
            this.gameState.getWorld().addPickUp(f.createPickUpObject(EXTRA_POINTS, new P2d(this.gameState.getWorld()), 1, new PickUpObjGraphicsComponent(true)));            
        }

        if(picksTaken%this.rand.nextInt(13, 20) == 0 && picksTaken != 0){           

            this.gameState.getWorld().addPickUp(f.createPickUpObject(CURSE, new P2d(this.gameState.getWorld()), 1, new CurseGraphicsComponent()));            
        }

        if(picksTaken%this.rand.nextInt(10, 25) == 0 && picksTaken != 0){           

            this.gameState.getWorld().addPickUp(f.createPickUpObject(SUPER, new P2d(this.gameState.getWorld()), 1, new SuperGraphicsComponent()));            
        }

        if(picksTaken%this.rand.nextInt(20, 25) == 0 && picksTaken != 0){           

            this.gameState.getWorld().addPickUp(f.createPickUpObject(LIFE, new P2d(this.gameState.getWorld()), 1, new LifeGraphicsComponent()));            
        }

        if(picksTaken%this.rand.nextInt(3, 5) == 0 && picksTaken != 0){           

            this.gameState.getWorld().addPickUp(f.createPickUpObject(BOMB, new P2d(this.gameState.getWorld()), 1, new BombGraphicsComponent()));            
        }

        if(picksTaken%this.rand.nextInt(2, 5) == 0 && picksTaken != 0){           

            gameState.getWorld().addSummoning(f.createSummoning(new P2d(gameState.getWorld()), 1, BLACK_BAT, gameState.getWorld()));            
        }

        if(picksTaken%this.rand.nextInt(6, 15) == 0 && picksTaken != 0){           

            gameState.getWorld().addSummoning(f.createSummoning(new P2d(gameState.getWorld()), 1, RED_BAT, gameState.getWorld()));            
        }
    ...
    public void createButtons(P2d pos, Button.Type type){
        ...
        if(null != type)switch (type) {
            ...
            case EXIT -> this.gameState.getWorld().addButton(f.createButton(pos, 40, 120, type, this));
                
            case HELP -> this.gameState.getWorld().addButton(f.createButton(pos, 40, 130, type, this));
                
            case MENU -> this.gameState.getWorld().addButton(f.createButton(pos, 40, 145, type, this));
                
            case START -> this.gameState.getWorld().addButton(f.createButton(pos, 40, 167, type, this));
